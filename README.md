...
| `dsh-client-ui-usage-stats` | 用量统计插件：设置页在「任务通知」下方，折叠本机全部会话日志（含子 Agent，可关闭），按「提供商 → 模型」「按模型」「按会话」三种视图展示 token 用量（输入 / 缓存读 / 缓存写 / 输出，推理单列但不重复计入）。同名模型由多个提供商提供时分别统计，「按模型」视图合并并列出全部提供商。数据只读、不折算费用、不做模型调用。 |
