...
- 更新（全部）：`dsh plugin --profile web update`；更新单个：`dsh plugin --profile web update dsh-client-ui-usage-stats`。
  三个包规格都未钉 commit、只钉默认分支，pnpm 会重新解析并拉回 `main` 最新状态；只替换
  profile 的 `node_modules`，不碰会话日志与设置。**注意生效范围**：`patchReload: live`
  只热重建客户端 bundle，所以只改了 `lib/client.js` 时刷新页面即生效；改了 host half
  （`lib/index.js`）则需重启 dsh 才生效。
