...
- 压缩在步骤边界检查阈值/间隔/预算后才可能调用 Jev；判断输入附带按时间排序的
  历史索引（非候选内容仅短摘录），候选原文必须完整放入状态；放不下就保留。
  `observe` 记录扣除替换标记开销后的预计净节省与调用次数；`active` 无净收益不提交。
  成功后以 `compaction/prune` + 表面替换提交，原文保留在日志中。
- 决策节点一律先取原生决策再增强：故障分类只在原生判定终止时于策略预算内补一次有界
  重试；路线选择只替换到白名单内的 provider/model（并按新模型重新门控）；风险判断只把
  `allow` 升级为 `ask`。PowerShell `Remove-Item` 等删除操作由本地规则优先识别；
  命令超出完整证据长度时回退原生判断，不会将截断片段标记为安全。
- 连续 3 次 Jev 调用失败熔断 5 分钟，期间全部走原生路径。
- 当前工具调用与结果仍作为原子单元筛选；“保留调用、仅缩短结果”尚未实现，
  因为现有表面替换与撤销流程只验证了整单元替换。审批被拒后是否再次尝试由宿主与
  代理决定，插件不会绕过或缓存宿主审批结果。
...
- **只记真实判断**：决策记录只在真的问过 Jev（`source:"jev"`）或确定性规则判定（`source:"rule"`）
  ...
  风险证据不足时只计 `riskInsufficientEvidence`，不记录为 `safe`；
